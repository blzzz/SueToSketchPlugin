...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/import-sue-chart.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* WEBPACK VAR INJECTION */(function(fetch) {var sketch = __webpack_require__(/*! sketch/dom */ "sketch/dom");

var UI = __webpack_require__(/*! sketch/ui */ "sketch/ui");

var chartTypes = ["arc", "arc-spider", "lollipop", "lollipop-horizontal", "line", "dotplot", "slope", "flow-arrows", "stackedBar", "stackedBar-labelOnTop", "stackedBar-column", "groupedBar", "groupedBar-labelOnTop", "groupedBar-column", "stackedBar-mekko", "waffleplot", "treemap", "area", "area-line", "scatterplot", "timeline"];
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  var selectedLayers = context.selection;
  var selectedCount = selectedLayers.length;

  var document = __webpack_require__(/*! sketch/dom */ "sketch/dom").getSelectedDocument(); // borrowed from https://github.com/bomberstudios/Cleanup-Useless-Groups
  ...
  function doInsertSVG(svgCode, layer, plotConfig) {
    ...
    var name = layer.name.split("|||||MASTERLAYER|||||")[0].trim(); // ensure original name
    ...
    var svgLayer = sketch.fromNative(importedLayer);
    layer.name = "".concat(name, " |||||MASTERLAYER||||| ").concat(svgLayer.id);
    svgLayer.name = "".concat(name, "'s SVG Layer |||||SLAVEGROUP||||| ").concat(JSON.stringify(plotConfig));
  ...
  function loadChart(_ref) {
    var chartType = _ref.chartType,
        data = _ref.data,
        signalSettings = _ref.signalSettings,
        width = _ref.width,
        height = _ref.height,
        style = _ref.style;
    ...
    return fetch("https://sue.st.nzz.ch/chart/".concat(style, "/").concat(chartType), {
      method: "POST",
      body: body,
      headers: {
      ...
      return context.document.showMessage(e);
    });
  }

  if (selectedCount !== 1) {
    context.document.showMessage("Select a Rectangle to convert it to a Sue Chart.");
  } else {
    var layer = sketch.fromNative(selectedLayers[0]);
    var masterLayerChunks = layer.name.split("|||||MASTERLAYER|||||");
    var isMasterLayer = masterLayerChunks.length === 2;
    var plotConfig = {};

    if (isMasterLayer) {
      var slaveGroupId = masterLayerChunks[1].slice(1);
      var slaveGroup = document.getLayerWithID(slaveGroupId);
      ...
      plotConfig = JSON.parse(slaveGroup.name.split("|||||SLAVEGROUP|||||")[1].slice(1));
      slaveGroup.remove();
      UI.message("resizing slave group");
    } else {
      var chartSelection = UI.getSelectionFromUser("What chart would you like to generate?", chartTypes);
      var ok = chartSelection[2];
      var chartType = chartTypes[chartSelection[1]];

      if (!ok) {
        UI.message("Bye! Next time!");
      }

      var data = UI.getStringFromUser("What's your data? Paste in TSV data from Excel or co, please.", "Monate\tWerte 2012\tWerte 2014\n1\t43\t91\n2\t81\t53\n3\t19\t87\n4\t52\t48");
      data = data.split("\n").map(function (row) {
        return row.split("\t");
      });
      plotConfig = {
    ...
    loadChart(plotConfig).then(function (svg) {
...
//# sourceMappingURL=import-sue-chart.js.map